...
- [HTTP的请求方法](#http的请求方法)
  ...
  - [HEAD](#head)
  - [POST](#post)
  - [POST 和 GET的区别](#post-和-get的区别)
  - [PUT](#put)
  - [DELETE](#delete)
  - [CONNECT](#connect)
  - [OPTIONS](#options)
  - [TRACE](#trace)
  - [HTTP/1.1之后增加的方法](#http11之后增加的方法)
- [HTTP1.0和HTTP1.1的一些区别](#http10和http11的一些区别)
- [HTTP1.0和1.1现存的一些问题](#http10和11现存的一些问题)
- [HTTPS应声而出](#https应声而出)
  - [HTTPS 的基础](#https-的基础)
  - [HTTP 向 HTTPS 演化的过程](#http-向-https-演化的过程)
- [SSL/TLS 握手机制概述](#ssltls-握手机制概述)
  - [基本的运行过程](#基本的运行过程)
    - [如何保证公钥不被篡改？](#如何保证公钥不被篡改)
    - [公钥加密计算量太大，如何减少耗用的时间？](#公钥加密计算量太大如何减少耗用的时间)
  - [握手阶段的详细过程](#握手阶段的详细过程)
    - [客户端发出请求（ClientHello）](#客户端发出请求clienthello)
    - [服务器回应（SeverHello）](#服务器回应severhello)
    - [客户端回应](#客户端回应)
    - [服务器的最后回应](#服务器的最后回应)
...
- [HTTP 1.0 1.1 2.0 的区别](#http-10-11-20-的区别)
- [Reference](#reference)
...
HTTP是一种超文本传输协议，其本身是无状态的协议，是一种常见的应用层协议。HTTP规定了客户端和服务端之间的内容格式
...
* 4xx 错误，表示请求可能出错，妨碍了服务器地处理
* 5xx 服务器错误，表示服务器在尝试处理请求时发生内部错误

## Cookie 和 Session

HTTP协议是一种无状态协议，我们可以使用Cookie和Session来保持会话状态。

用户发起请求，服务端收到请求处理后可以生成一个sessionid，并且将sessionid存入cookie中返回给客户端，将session的内容存储在服务器上。在下一次的请求中，客户端带着cookie来请求服务器，服务端冲cookie中取出sessionid，实现了用户会话状态的保持。

这样做有一个缺点就是将一些东西存在了服务器上，在用户量较大的情况下，服务器容量会不足。实际情况中，经常是将所需要的绘画状态，比如说登陆状态直接存入cookie中并返回给客户端，下次请求时，服务端直接取出cookie中的信息和参数信息进行比较，保持http会话状态

一句话总结就是，session存在服务端，cookie保存在客户端，并且cookie有大小限制
...
HTTP 1.0 是一种无状态无连接的协议。浏览器的每次请求都需要与服务器建立一个TCP连接，服务器处理完成后立即断开TCP连接（无连接），服务器不跟踪每个客户端也不记录过去的请求（无状态）。

HTTP 1.1 中默认使用 ```Connection: keep-alive```，避免了连接建立和释放的开销。但服务器必须按照客户端请求的先后顺序依次返送相应的结果，以保证客户端能够区分出每次请求的相应内容。通过```Content-Length```字段来判断当前请求的数据是否已经全部接受。不允许同时存在两个并行的响应

HTTP 2.0 协议新增了二进制分帧，多路复用，头部压缩和服务器推送等功能，进一步提高了传输效率
