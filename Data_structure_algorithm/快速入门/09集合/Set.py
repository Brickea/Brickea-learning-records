...
class Set(HashTable):
    ...
    def __and__(self,other_table):
        # 实现and操作
        result = Set()
        for item in other_table:
            if item in self:
                result.add(item)
        for item in self:
            if item in other_table:
                result.add(item)
        return result

    def __or__(self,other_table):
        # 实现or操作
        result = Set()
        for item in other_table:
            result.add(item)
    ...
    def __sub__(self,other_table):
        # 实现差集操作
        result = Set()
        for item in self:
            if item not in other_table:
                result.add(item)
        return result
...
def test_set():
    ...
    assert list(s - other_s) == [2]

    assert list(s&other_s) == [1,3]

    assert list(s|other_s) == [1,2,3]
